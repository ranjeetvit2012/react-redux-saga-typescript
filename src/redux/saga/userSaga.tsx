...
function* userRegisterRequestData(data: any): Generator<any, void, any> {

  try {
  //  const userRes = yield call(apiCall, 'POST', '/user/register', data?.payload, false);
  const userRes = yield call(() => apiCall('POST', '/user/register', data?.payload, ""));
...
function* userLoginRequestData(data:any):Generator<any,void,any> {
  try {
    const loginRes = yield call(()=>apiCall("POST", '/user/login', data?.payload,""))
    ...
    console.log("  ---loginRes user saga for login-catch ",error)
